    ...
    public class GASDebugWindow : EditorWindow
    {
        ...
        private void DrawAttributeSet()
        {
            ...
            foreach (var field in fields)
            {
                ...
                {
                    if (field.GetValue(attributeSet) is GameplayAttribute attribute)
                    {
            ...
            foreach (var prop in properties)
            {
                ...
                {
                    if (prop.GetValue(attributeSet) is GameplayAttribute attribute)
                    {
        ...
        private void DrawAbilities()
        {
            ...
            foreach (var ability in grantedAbilities)
            {
                ...
                if (isOnCooldown)
                {
                    ...
                    float maxCooldown = ability.cooldownDuration;
        ...
        private void DrawCooldowns()
        {
            ...
            foreach (var kvp in abilityCooldowns)
            {
                ...
                float maxCooldown = kvp.Key.cooldownDuration;
                float currentRemaining = selectedASC.GetAbilityCooldownRemaining(kvp.Key);
                float progress = maxCooldown > 0 ? 1f - (currentRemaining / maxCooldown) : 1f;

                Rect rect = EditorGUILayout.GetControlRect(false, 18);
                EditorGUI.ProgressBar(rect, progress, $"{currentRemaining:F1}s");
