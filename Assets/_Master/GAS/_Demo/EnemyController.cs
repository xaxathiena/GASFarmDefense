...
namespace FD
{
    ...
    public class EnemyController: IAbilitySystemComponent
    {
        ...
        public void OnSetup(EnemyView enemyView, EnemyData enemyData)
        {
            ...
            // Set initial health from enemyData
            if (enemyData != null)
            {
                attributeSet.MaxHealth.SetBaseValue(enemyData.InitialHealth);
                attributeSet.Health.SetCurrentValue(enemyData.InitialHealth);
                attributeSet.Armor.SetBaseValue(enemyData.InitialArmor);
                attributeSet.armorType = enemyData.ArmorType;
            }
            
            // Initialize profiler
            profiler = new ASCPerformanceProfiler(acs, $"Enemy_{currentCount}");
            
            // Subscribe to death event
            attributeSet.Health.OnValueChanged += OnHealthChanged;
        ...
        {
            // Auto destroy when health reaches 0
            if (newValue <= 0 && oldValue > 0)
            {
                debug.Log($"Enemy {DisplayName} died! Auto destroying...", Color.red);
                Destroy();
            }
        }
