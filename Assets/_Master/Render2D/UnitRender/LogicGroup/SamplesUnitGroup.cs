...
namespace Abel.TowerDefense.Logic
{
    ...
    public class SimpleLoopGroup : UnitGroupBase
    {
        ...
        {
            LogicData = new NativeArray<UnitLogicData>(profile.maxCapacity, Allocator.Persistent);
        ...
        protected void OnSpawnUnit(ref UnitLogicData logic, ref UnitRenderData render)
        {
    ...
    public class AggressiveFSMGroup : UnitGroupBase
    {
        ...
        public override void Spawn(Vector2 position)
        {
            if (ActiveCount >= profile.maxCapacity) return;
            int id = ActiveCount;
